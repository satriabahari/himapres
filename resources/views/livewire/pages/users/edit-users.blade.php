                    ...
                    <div class="">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="role" name="name"
                                value="{{ $user->name }}" placeholder="Enter Roles ..." disabled>

                            <label for="role" class="form-label">E-mail</label>
                            <input type="text" class="form-control" id="role" name="name"
                                value="{{ $user->email }}" placeholder="Enter Roles ..." disabled>

                            <label for="role" class="form-label mb-0">Role</label>
                            @if ($user->roles)
                                <div class="d-flex">
                                    @foreach ($user->roles as $user_role)
                                        <button class="btn-sm btn-danger-light mt-4 mb-4 mx-2" type="button"
                                            wire:click="removeRole({{ $user->id }}, {{ $user_role->id }})"
